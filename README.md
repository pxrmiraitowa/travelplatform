...
前台 `行程规划` 模块新增了 `AI 生成行程` 入口，当前版本特性如下：

- 仅支持结构化输入：`目的地`、`停留天数`、`出发日期`、`旅游偏好`
- 可在生成后预览按天拆分的景点推荐结果
- 支持一键保存到现有 `行程规划` 中，保存后仍可继续手动编辑
- 保存后的计划会标记为 `AI` 来源
...
- 已预留第三方 AI 调用接口，仅用于未来做景点排序优化和推荐理由补充
- 当前版本未实际启用第三方 AI 调用，演示时仍以本地规则结果为准
