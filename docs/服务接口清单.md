...
| 内部接口 | 微服务之间调用的 `/api/internal/**` 接口，不直接暴露给前端 |
...
| 方法 | 路径 | 功能 | 是否需要管理员 | 说明 |
| --- | --- | --- | --- | --- |
| `POST` | `/api/auth/login` | 管理员登录 | 否 | 使用统一登录接口，JWT 携带管理员角色 |
| `GET` | `/api/admin/dashboard` | 管理后台仪表盘 | 是 | 聚合用户、商品和订单统计 |
...
| 方法 | 路径 | 调用方 | 功能 | 说明 |
| --- | --- | --- | --- | --- |
| `GET` | `/api/internal/products/snapshot` | `order-service`、`content-trip-service` | 查询商品快照 | 使用 `productType`、`productId` 和可选规格参数 |
| `POST` | `/api/internal/products/stock/deduct` | `order-service` | 扣减库存 | 下单时原子扣减 |
| `POST` | `/api/internal/products/stock/restore` | `order-service` | 恢复库存 | 取消、退款或订单写入失败时调用 |
| `GET` | `/api/internal/coupons/settlement` | `order-service` | 优惠券结算 | 可选优惠券校验，返回原价、优惠金额和应付金额 |
| `GET` | `/api/internal/attractions` | `content-trip-service` | 查询景点候选 | 按 `city` 或 `ids` 查询，内容服务不读取景点表 |
...
| 方法 | 路径 | 功能 | 是否需要登录 | 说明 |
| --- | --- | --- | --- | --- |
| `POST` | `/api/orders` | 创建统一订单 | 是 | `productType` 区分航班、车次、酒店房型和旅游产品 |
| `GET` | `/api/orders` | 查询当前用户订单列表 | 是 | 支持业务类型和状态筛选 |
| `GET` | `/api/orders/{id}` | 查询订单详情 | 是 | 校验订单归属 |
| `POST` | `/api/orders/{id}/cancel` | 取消订单 | 是 | 取消成功后调用产品服务恢复库存 |
| `POST` | `/api/orders/{id}/pay` | 模拟支付 | 是 | 课程演示状态流转，不接真实支付 |
| `POST` | `/api/orders/{id}/refund` | 模拟退款 | 是 | 已支付订单退款并恢复库存 |
| `POST` | `/api/orders/{id}/complete` | 完成订单 | 是 | 演示用状态流转 |
...
| 方法 | 路径 | 调用方 | 功能 | 说明 |
| --- | --- | --- | --- | --- |
| `GET` | `/api/internal/orders/{orderId}/review-context` | `content-trip-service` | 评价前订单校验 | 校验归属、完成状态并返回商品快照 |
| `GET` | `/api/internal/orders/reviewable` | `content-trip-service` | 查询可评价订单 | 按用户分页查询已完成订单 |
...
| `GET` | `/api/shares/mine` | 当前用户分享列表 | 是 | 查询自己发布的分享 |
...
| `GET` | `/api/orders/reviewable` | 查询可评价订单 | 是 | 内容服务调用订单服务获取列表 |
| `GET` | `/api/orders/{id}/review` | 查询订单评价 | 是 | 查询当前用户某订单的评价 |
...
| `/api/admin/dashboard` | `user-service` | 后台统计聚合 |
...
| 调用方 | 被调用方 | 接口 | 场景 | 失败处理 |
| --- | --- | --- | --- | --- |
| `order-service` | `product-service` | `GET /api/internal/products/snapshot` | 下单前查询商品价格、库存、状态 | 商品服务不可用时下单失败 |
| `order-service` | `product-service` | `POST /api/internal/products/stock/deduct` | 下单时扣减库存 | 库存不足或扣减失败时不创建订单 |
| `order-service` | `product-service` | `POST /api/internal/products/stock/restore` | 取消、退款或订单写入失败补偿 | 恢复失败时记录日志并返回明确提示 |
| `content-trip-service` | `order-service` | `GET /api/internal/orders/{id}/review-context` | 提交评价前校验订单 | 订单服务不可用时拒绝评价 |
| `content-trip-service` | `order-service` | `GET /api/internal/orders/reviewable` | 查询可评价订单 | 调用失败时返回明确提示 |
| `content-trip-service` | `product-service` | `GET /api/internal/attractions` | 行程生成时查询景点候选 | 失败时使用本地规则数据 |
| `content-trip-service` | `product-service` | `GET /api/internal/products/snapshot` | 价格提醒查询商品当前价格 | 展示时失败则显示当前价格暂不可用 |

## 10. 接口设计约束

1. 所有外部接口继续使用 `/api` 前缀，降低前端改造成本。
2. 服务间接口统一使用 `/api/internal` 前缀，不经网关暴露给前端。
