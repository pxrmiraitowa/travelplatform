...
| `travel_order` | `order-service` | 统一订单、商品快照、订单状态 |
...
| `orders` | `order-service` | `travel_order` | 保存订单号、用户 ID、业务类型、规格、商品快照、金额和订单状态 | 是 | `content-trip-service` 通过订单评价上下文接口访问 |
...
| 表名 | 说明 |
| --- | --- |
| `orders` | 统一订单及商品快照；不保留按商品类型拆分的明细表 |
...
3. 不跨服务直接联表查询。
4. 跨服务引用统一保存业务 ID。
5. `orders` 表保存商品名称、规格、单价等快照，避免商品信息修改后影响历史订单展示。
6. CI 部署后执行 `scripts/verify-table-ownership.sh`，实际表清单与本表不一致时流水线失败。
7. 其他服务需要商品、订单或用户信息时，必须通过对应服务提供的接口访问。
...
| `order-service` | `Order` 及对应 Mapper；不同商品类型使用统一快照字段 |
