...
服务间接口统一使用 `/api/internal/**` 前缀，主要供服务内部调用，不经网关暴露给前端页面。
...
| 调用方 | 被调用方 | 业务场景 | 调用目的 |
| --- | --- | --- | --- |
| `order-service` | `product-service` | 创建订单 | 查询商品快照、校验价格库存、可选优惠券结算、扣减库存 |
...
6. 如果请求携带 `couponId`，`order-service` 调用 `product-service` 校验优惠券并计算优惠后金额；未携带则按原价结算。
7. `order-service` 调用 `product-service` 扣减库存。
8. 扣库存成功后，`order-service` 将商品和规格快照写入统一 `orders` 表。
9. `order-service` 返回订单创建成功结果。

### 4.3 建议接口

| 方法 | 接口 | 调用方 | 被调用方 | 说明 |
| --- | --- | --- | --- | --- |
| `GET` | `/api/internal/products/snapshot?productType={type}&productId={id}` | `order-service` | `product-service` | 查询商品快照 |
| `GET` | `/api/internal/coupons/settlement?productType={type}&couponId={couponId}&originalAmount={amount}` | `order-service` | `product-service` | 可选优惠券校验和金额结算 |
| `POST` | `/api/internal/products/stock/deduct` | `order-service` | `product-service` | 原子扣减库存 |
...
| 库存不足 | 下单失败，返回“库存不足” |
| 优惠券不存在、不适用、过期或未达门槛 | 下单失败，返回对应业务提示，不创建订单 |
| 扣库存失败 | 下单失败，不创建订单 |
...
| 方法 | 接口 | 调用方 | 被调用方 | 说明 |
| --- | --- | --- | --- | --- |
| `POST` | `/api/internal/products/stock/restore` | `order-service` | `product-service` | 恢复库存 |
...
4. `order-service` 返回订单是否存在、是否属于当前用户、是否已完成及商品快照。
5. 校验通过后，`content-trip-service` 写入 `review` 表。
6. `content-trip-service` 依靠 `review.order_id` 唯一约束防止重复评价并返回结果。

### 6.3 建议接口

| 方法 | 接口 | 调用方 | 被调用方 | 说明 |
| --- | --- | --- | --- | --- |
| `GET` | `/api/internal/orders/{id}/review-context?userId={userId}` | `content-trip-service` | `order-service` | 校验订单并返回评价上下文 |
| `GET` | `/api/internal/orders/reviewable?userId={userId}` | `content-trip-service` | `order-service` | 查询可评价订单 |
...
| 方法 | 接口 | 调用方 | 被调用方 | 说明 |
| --- | --- | --- | --- | --- |
| `GET` | `/api/internal/attractions?city={city}` | `content-trip-service` | `product-service` | 查询目的地景点候选 |
...
| 方法 | 接口 | 调用方 | 被调用方 | 说明 |
| --- | --- | --- | --- | --- |
| `GET` | `/api/internal/products/snapshot?productType={type}&productId={id}` | `content-trip-service` | `product-service` | 查询商品当前价格和状态 |
...
当前实现由 `SERVICE_CONNECT_TIMEOUT_MS` 和 `SERVICE_READ_TIMEOUT_MS` 配置，默认连接超时 2 秒、读取超时 3 秒：
